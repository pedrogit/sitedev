...
/*
+----------------------------------------------------------------------+
| Copyright 2015 Hans Bracker. http://www.softflow.uk
...
+----------------------------------------------------------------------+
| fox.php for PmWiki
| A form processing script for PmWiki 2.2.76 and above, and PHP 7.2 and above
| Instructions on how to use it see https://www.pmwiki.org/wiki/FoxDocumentation/
+----------------------------------------------------------------------+
| Other contributors:
...
+----------------------------------------------------------------------+
*/
$RecipeInfo['Fox']['Version'] = '2024-07-11'; 
...
SDV($FoxSortTargets, 1); //for multiple target page processes target pages get sorted by name, then processed
SDV($FoxJSFormCheckFunction, 'FoxJSFormCheck'); //function to inject javascript form check
...
function FoxMarkup($m) {
    ...
    $out.= "<form  name='$name' action='{$PageUrl}' method='post' ".
        (isset($opt['newwin']) ? "target='_blank' " : "").
        (isset($opt['upload'])||isset($opt['uptarget']) ? "enctype='multipart/form-data' " : "").
...
function FoxEditTemplate($pagename, &$page, &$new) {
    global $FoxDebug;
    //DEBUG//
    if ($FoxDebug>4) show($_SESSION["FoxTempPageText"],'SESSION FoxTempPageText');
    if (@$new['text'] > '') return '';
...
function FoxHandlePost($pagename, $auth) {
    ...
    //FoxCSV pre-processing (function in foxcsv.php)
    if (function_exists('fxc_Preprocess_Input') && isset($fx['foxaction']) && $fx['foxaction']=='csv')
        fxc_Preprocess_Input($pagename, $fx); 
...
function FoxRedirectFmt ($pagename, $targets, $fx) {
    ...
    $arg = ''; $anch = '';
    if (isset($fx['redir'])) $redir = $fx['redir']; 
    else if (isset($fx['cancelredir'])) $redir = $fx['cancelredirect'];
    else $redir = $pagename;
    if ($redir=='0') $redir = $pagename;
    if (substr($redir,0,4)=="http") {
        ...
        if ($fx['redir']==='1') {
            ...
            $rr = explode("?", $redir); 
            $aa = explode("#", $rr[0]);
            if (isset($aa[1])) $anch = "#".$aa[1];
            $pname = FoxGroupName($pagename, $fx, $rr[0]);
            if (isset($rr[1])) $arg = "?".$rr[1]; 
        }
...
function FoxTargetList ($pagename, &$fx) {
    ...
    if (!isset($fx[':target']) &&  !isset($fx['target']) && !isset($fx['newedit'])) 
        if ((isset($fx['template']) && $fx['template']!=0) || isset($fx['foxtemplate']))
            if (!isset($fx['foxaction']) || (isset($fx['foxaction']) && $fx['foxaction']!='copy' && !($fx['foxaction']=='replace' && $fx['put']=='overwrite')))
                $fx['target'] = $pagename;

    //'display' by default has current page as target 
    if (isset($foxaction) && $fx['foxaction']=='display' && !isset($fx['target'])) $fx['target'] = array($pagename);
    ...
    elseif (isset($fx['upload']) && $fx['upload']==1) {
        if (isset($fx['target'])) {
    ...
    if (isset($fx['ptvupdate']) && $fx['ptvupdate']==1 && !isset($fx['ptvtarget']) && isset($fx['target']))
    ...
    if ($GLOBALS['FoxSortTargets']==0 OR (isset($fx['notargetsort']) && $fx['notargetsort']==1))
        usort($to, "FoxTargetActionSort");
    else usort($to, "FoxTargetSort");
...
## sort $to array by foxaction only
function FoxTargetActionSort($a ,$b) {
    $order = array('copy','add','replace','ptv','display');
    return array_search($a['foxaction'], $order) - array_search($b['foxaction'], $order);
} //}}}
...
function FoxProcessTargets($pagename, $fx, $to) { 
    ...
    foreach ($to as $tn => $tacts) {
        ...
        foreach ($tacts as $idx => $tg) {
            ...
            //add, replace, csv, ptv, copy: load page file for saving
            if ($act=='add' || $act=='replace' || $act=='csv' || $act=='ptv' || $act=='copy') {
                ...
                if ($permit == true) switch ($act) {
                    ...
                    case 'csv':     if (function_exists('FoxCSV_Update')) 
                                        $text = FoxCSV_Update($pagename, $text, $template, $fx); break;
            ...
            if ($idx==count($tacts)-1 && isset($text) && isset($pagetext)) {
                //DEBUG
...
function FoxLoadTemplate($pagename, $fx, $tg) {
    $tplname = (isset($tg['template']))? $tg['template'] : '';
    ...
    switch($tplname) {
        ...
        default :
            ...
            if (isset($tg['foxtemplate']) && $tg['foxtemplate']!='') return Fox_htmlspecialchars_decode($tg['foxtemplate']); 
...
function FoxAddText($pn, $text, $template, $fx, $tg) {
    global $FoxMsgFmt, $FoxDebug; if ($FoxDebug>0) echo "<pre><b>FoxAddText></b></pre>"; //DEBUG//
...
function FoxTemplateEngine($pn, $template, $fx, $tg, $linekeyseed=NULL, $caller=NULL) {
    ...
    $result = implode("\n",$result);
    if ($FoxDebug>6) echo "<br/>RESULT from ENGINE: $result <br/>";
    return $result;
} //}}}
...
function FoxVarRepRecursive(&$v, $k, $d) {
    global $FoxMaxIterations,$FoxDebug; if ($FoxDebug>4) echo "FoxVarRepRecursive>";
    if ($d['tg']=='') return;
    SDV($FoxMaxIterations, 100);
    static $cnt = 0;
    $v = FoxVarReplace($d['pn'], $d['fx'], $d['tg'], $v);
    $cnt++;
    if (isset($d['tg']['t_idx']))
        $maxcnt = $FoxMaxIterations + $d['tg']['t_idx'];
        else $maxcnt = $FoxMaxIterations;
    if ($cnt >= $maxcnt)
...
function FoxVarReplace($pn, $fx, $tg, $str) {
    ...
    if ($FoxDebug>6) echo "<pre><b>VNEW=</b><br/>".$str."</pre>"; //DEBUG//
    return $str;
} //}}}
...
function FoxValue($fx, $tg, $fullvar, $var, $idx=NULL, $schars=NULL, $quote1=NULL, $quote2=NULL) {
    ...
    if (isset($tg['foxaction']) && ($tg['foxaction']=='add' || $tg['foxaction']=='csv')) $fullvar = '';
    return $fullvar;
} //}}}
...
function FoxSecurityCheck($pagename, $targets, &$fx) {
    ...
    if (!isset($fx['post']) AND !isset($fx['post2']) AND !isset($fx['post_x']) AND !isset($fx['postdraft']) AND !isset($fx['cancel']) AND !isset($fx['preview'])) {
    ...
    foreach ($check as $opt) {
        foreach ($opt['names'] as $n) {
            ...
            if ($inclp && !preg_match("/$inclp/is", $fx[$n]) 
                ...
                $FoxMsgFmt[$n] = isset($opt['msg']) ? $opt['msg'] : "$[Invalid parameter:] $n";
                $FoxCheckError[] = $n;
            }
            if (@$opt['if'] && !CondText($pagename, 'if '.$opt['if'], 'yes')) {
                $FoxMsgFmt[$n] = isset($opt['msg']) ? $opt['msg'] : "$[Input condition failed]";
                $FoxCheckError[] = $n;
            }
        }
    }
...
if (!function_exists('show')) {
    function show($arr,$lbl=''){echo "<pre><b>$lbl</b>";print_r($arr);echo "</pre>";} 
} //}}}
if (!function_exists('vdu')) {
    function vdu($arr,$lbl=''){echo "<pre><b>$lbl</b>";var_dump($arr);echo "</pre>";} 
} //}}}
