...
/*  Copyright 2007-2023 by D.Faure (dominique.faure@gmail.com) and
    ...
    The operations defined by this recipe include:
      ...
      cat                     - concatenate parameters
      echo                    - output parameters as-is
...
*/
$RecipeInfo['MarkupExprPlus']['Version'] = '2023-02-24';
...
SDVA($MarkupExpr, array(
  ...
  'test' => 'MEP_test($pagename, $args)',
  'if' => '($args[0]) ? $args[1] : (isset($args[2]) ? $args[2] : "")',
  'sprintf' => 'call_user_func_array("sprintf", $args)',
  'nomarkup' => 'preg_replace("/(<[^>]+>|\r\n?|\n\r?)/", "", MarkupToHTML($pagename, $args[0], array("escape" => 0)))',
...
if (! function_exists('PSFT') && !IsEnabled($EnableExprOriginalFtime, 0)) {
...
##  MEP_test handle {(test ...)}
##
function MEP_test($pagename, $args) {
  foreach ($args as $i => $arg)
    if (strpos($arg, ' ') !== false)
      $args[$i] = strpos($arg, '"') !== false ? "'$arg'" : "\"$arg\"";
  array_unshift($args, 'if');
  return CondText($pagename, implode(' ', $args), "TRUE") ? "1" : "0";
}
...
function MEP_unaccent($s) {
  ...
  if (strpos($s = htmlentities($s, ENT_QUOTES, $MEP_Encoding), '&') !== false)
    $s = html_entity_decode(preg_replace('/&([a-z]{1,2})(?:acute|caron|cedil|circ|grave|lig|orn|ring|slash|tilde|uml);/i', '$1', $s), ENT_QUOTES, $MEP_Encoding);
  return $s;
}
...
if (IsEnabled($EnableExprVarManip, 0)) {
  ...
  SDVA($MarkupExpr, array(
    ...
    'pagevar' => 'MEP_resolve_vars($pagename, "{".$args[0]."}")',
  ));
  ...
  ##
  function MEP_resolve_vars($pagename, $expr) {
    global $MarkupTable, $MEP_Cfg;
    if (isset($MarkupTable[$MEP_Cfg['httpvars']]))
      $expr = MEP_parse_vars($pagename, $MEP_Cfg['httpvars'], $expr);
    return MEP_parse_vars($pagename, '{$var}', $expr);
  }
...
if(IsEnabled($EnableExprMultiline, 0) || IsEnabled($EnableExprVarManip, 0)) {
  ...
  function MEP_cb_keep_m0_p($m) { return Keep($m[0],'P'); }
  ...
  function MarkupExpressionPlus($pagename, $expr) {
    ...
    if (IsEnabled($EnableExprVarManip, 0))
      $expr = MEP_resolve_vars($pagename, $expr);
    $expr = preg_replace_callback($MEP_Cfg['KeepRe'], "MEP_cb_keep_m2_p", $expr);
    $expr = preg_replace_callback('/\\(\\W/', 'MEP_cb_keep_m0_p', $expr);
